        ...
        if re.findall(r"^\+(?:[0-9]●?){6,14}[0-9]$", phone):
            ...
            if old.exists():
                ...
                if count > 3:
                    data = {}
                    data['error'] = False
                    data['success_msg'] = 'Maximum otp limits reached. Kindly support our customer care or try with different number'
                    return JsonResponse(data)
                old.save()
                ...
                data = {}
                data['error'] = False
                data['success_msg'] = 'Successfully send OTP'
                return JsonResponse(data)
            else:
                count = count + 1
                ...
                data = {}
                data['error'] = False
                data['success_msg'] = 'OTP sent Successfully'
                return JsonResponse(data)
        ...
        if phone and otp_sent:
            ...
            if old.exists():
                ...
                if str(otp) == str(otp_sent):
                    ...
                    data = {}
                    data['error'] = True
                    data['error_msg'] = 'Otp matched'
                    return JsonResponse(data)
                else:
                    data = {}
                    data['error'] = False
                    data['error_msg'] = 'Otp not matched'
                    return JsonResponse(data)
            else:
                data = {}
                data['error'] = True
                data['error_msg'] = 'Otp not recived'
                return JsonResponse(data)
